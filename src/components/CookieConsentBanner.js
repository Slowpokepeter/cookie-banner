...
const CookieConsentBanner = ({
  ...
  const initializeGTM = () => {
    ...
    if (preferences.analytics) {
      window.dataLayer.push({ event: 'analyticsAccepted' });
    }
    if (preferences.marketing) {
      window.dataLayer.push({ event: 'marketingAccepted' });
    }
  };
    ...
    <div className={`cookie-consent-popup ${style} ${position}`}>
      ...
      {!showCustomize ? (
        <>
          ...
          <p className="p-cookies">We use cookies to improve our services. Please select your preferences:</p>
